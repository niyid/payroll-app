 ...
 * ID of each face.
 */
public final class LifeProofActivity extends AppCompatActivity implements SensorEventListener, PhotoUploader {
	private static final String LOG_TAG = "LifeProofActivity";
	...
	private LifeDetector mFaceGraphic;
	...
	static PollingNotifierTimerTask timerTask;
	...
	private void createCameraSource() {
		...
		FaceDetector detector = new Builder(context).setLandmarkType(FaceDetector.ALL_LANDMARKS).setMode(FaceDetector.ACCURATE_MODE).setClassificationType(FaceDetector.ALL_CLASSIFICATIONS).setProminentFaceOnly(true).build();
		...
		timerTask = new PollingNotifierTimerTask(mPreview.getContext());
		timerTask.schedule();
		Log.i(LOG_TAG, "Scheduled timer...");
	...
	private class GraphicFaceTracker extends Tracker<Face> {
		...
		GraphicFaceTracker(GraphicOverlay overlay) {
			mOverlay = overlay;
			mFaceGraphic = new LifeDetector(overlay);
		}
...
//					+ (lightQuality / 250)) * ((float) face.getLandmarks().size() / 8.0) / 5;
		...
		public double computeProfileInput(Face face) {
			return (face.getIsSmilingProbability() + face.getIsLeftEyeOpenProbability() + face.getIsRightEyeOpenProbability() + (1 - face.getEulerY())
					+ (1 - face.getEulerZ())) / 5;
		}
	}
	...
	public void upload(File[] photos) {
		if (NetworkStatus.getInstance(LifeProofActivity.this).isOnline(LifeProofActivity.this)) {
			new LifeProofTask(this, photos).execute();
		} else {
			photoTaken = false;
			LifeProofActivity.this.displayAlert("There is no connection to the server - You do not have Internet access.", "Error", false);
		}
	}
	 ...
	 * the user.
	 */
	public class LifeProofTask extends AsyncTask<Void, Void, String> {

		private final LifeProofActivity activity;
		private final File[] photos;

		LifeProofTask(LifeProofActivity activity, File[] photos) {
		...
		protected String doInBackground(Void... params) {
			...
			try {
				...
				Object val = models.call("execute_kw", MainActivity.db, MainActivity.userId, MainActivity.password, "hr.employee", "activate_payment",
		...
		protected void onPostExecute(final String msg) {
			...
			if (msg != null) {
				try {
					Long pid = Long.parseLong(msg);
					MainActivity.pid = pid;
					MainActivity.pendingMsg = true;
					MainActivity.pendingMsgTxt = "Your payment has been activated; you will receive a payment confirmation notification shortly.";
					MainActivity.pendingMsgTitle = "Success";
					Intent intent = new Intent(LifeProofActivity.this, MainActivity.class);
					LifeProofActivity.this.startActivity(intent);
					finish();
