...
public class CustomCameraSource {
	 ...
	 * use an OpenGL context, we can choose any ID we want here.
	 */
	...
	@StringDef({ Camera.Parameters.FOCUS_MODE_CONTINUOUS_PICTURE, Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO, Camera.Parameters.FOCUS_MODE_AUTO,
	...
	@StringDef({ Camera.Parameters.FLASH_MODE_ON, Camera.Parameters.FLASH_MODE_OFF, Camera.Parameters.FLASH_MODE_AUTO, Camera.Parameters.FLASH_MODE_RED_EYE,
	 ...
	 * from the device. See {@link Frame.Metadata#getRotation()}.
	 */
	private int mRotation;

	private Size mPreviewSize;

	// These values may be requested by the caller. Due to hardware limitations,
	// we may need to
	...
	public static class Builder {
		 ...
		 * detector. Camera preview images will be streamed to the associated
		 * detector upon starting the camera source.
		 */
		 ...
		 * requested value is not not available, the best matching available
		 * value is selected. Default: 30.
		 */
		...
		/**
		 * Sets the desired width and height of the camera frames in pixels. If
		 * the exact desired values are not available options, the best matching
		 * available options are selected. Also, we try to select a preview size
		 * which corresponds to the aspect ratio of an associated full picture
		...
		public Builder setRequestedPreviewSize(int width, int height) {
			...
			// devices can support. We bound this to avoid int overflow in the
			// code later.
	...
	public interface PictureCallback {
		 ...
		 * format of the data is a jpeg binary.
		 */
	 ...
	 *
	 * @throws IOException
	 *             if the camera's preview texture or display could not be
	 *             initialized
	 */
	...
	public CustomCameraSource start() throws IOException {
		synchronized (mCameraLock) {
			...
			// SurfaceTexture was introduced in Honeycomb (11), so if we are
			// running and
			...
			params.setWhiteBalance(Parameters.WHITE_BALANCE_TWILIGHT);
	 ...
	 * @throws IOException
	 *             if the supplied surface holder could not be used as the
	 *             preview display
	 */
	@RequiresPermission(Manifest.permission.CAMERA)
	 ...
	 * source and release the resources of the underlying detector.
	 */
	public void stop() {
		synchronized (mCameraLock) {
			...
			if (mProcessingThread != null) {
				try {
					// Wait for the thread to complete to ensure that we can't
					// have multiple threads
					// executing at the same time (i.e., which would happen if
					// we called start too
					// quickly after stop).
			...
			if (mCamera != null) {
				...
				try {
					// We want to be compatible back to Gingerbread, but
					// SurfaceTexture
					// wasn't introduced until Honeycomb. Since the interface
					// cannot use a SurfaceTexture, if the
					// developer wants to display a preview we must use a
					// SurfaceHolder. If the developer doesn't
					// want to display a preview we use a SurfaceTexture if we
					// are running at least Honeycomb.
	 ...
	 * source should have been activated previously with {@link #start()} or
	 ...
	 *         or the camera is not yet created.
	 ...
	 * should be called. If the camera does not support auto-focus, it is a
	 * no-op and {@link AutoFocusCallback#onAutoFocus(boolean)} callback will be
	 * called immediately.
	 * <p/>
	 * <p>
	 ...
	 * the default. If the camera does not support auto-focus, this is a no-op.
	 *
	 ...
	 * @param cb
	 *            the callback to run
	 * @return {@code true} if the operation is supported (i.e. from Jelly
	 *         Bean), {@code false} otherwise
	 */
	 ...
	 * automatically turn the camera preview back on after the picture has been
	 * taken.
	 */
	...
	/**
	 * Wraps the camera1 auto focus move callback so that the deprecated API
	 * isn't exposed.
	 */
	@TargetApi(Build.VERSION_CODES.JELLY_BEAN)
	...
	private Camera createCamera() {
		...
		// detection
		// two for the frames that the camera uses to populate future preview
		// images
	 ...
	 *
	 * @param facing
	 *            the desired camera (front-facing or rear-facing)
	 */
	 ...
	 * because these need to have the same aspect ratio. On some hardware, if
	 * you would only set the preview size, you will get a distorted image.
	 *
	 * @param camera
	 *            the camera to select a preview size from
	 * @param desiredWidth
	 *            the desired width of the camera preview frames
	 * @param desiredHeight
	...
	private static SizePair selectSizePair(Camera camera, int desiredWidth, int desiredHeight) {
		...
		// differences between
		// the desired values and the actual values for width and height. This
		// is certainly not the
		// only way to select the best size, but it provides a decent tradeoff
		// between using the
	 ...
	 * be set to a size that is the same aspect ratio as the preview size or the
	 * preview may end up being distorted. If the picture size is null, then
	 * there is no picture size with the same aspect ratio as the preview size.
	 */
	 ...
	 * aspect ratio. If there is a corresponding picture size of the same aspect
	 * ratio, the picture size is paired up with the preview size.
	 * <p/>
	...
	private static List<SizePair> generateValidPreviewSizeList(Camera camera) {
		...
		for (android.hardware.Camera.Size previewSize : supportedPreviewSizes) {
			...
			// higher resolutions.
			// We choose the highest resolution in order to support taking the
			// full resolution
			// picture later.
	 ...
	 * @param desiredPreviewFps
	 *            the desired frames per second for the camera preview frames
	...
	private int[] selectPreviewFpsRange(Camera camera, float desiredPreviewFps) {
		...
		// differences between
		// the desired value and the upper and lower bounds of the range. This
		// may select a range
	 ...
	 *
	 * @param parameters
	 *            the camera parameters for which to set the rotation
	 * @param cameraId
	 *            the camera id to set rotation based on
	 */
	...
	private byte[] createPreviewBuffer(Size previewSize) {
		...
		if (!buffer.hasArray() || (buffer.array() != byteArray)) {
			// I don't think that this will ever happen. But if it does, then we
			// wouldn't be
	 ...
	 * the camera. As these frames come in, the most recent frame is held onto
	 * as pending. As soon as detection and its associated processing are done
	 * for the previous frame, detection on the mostly recently received frame
	 * will immediately start on the same thread.
	 */
	private class FrameProcessingRunnable implements Runnable {
		...
		void setNextFrame(byte[] data, Camera camera) {
			synchronized (mLock) {
				...
				// Timestamp and frame ID are maintained here, which will give
				// downstream code some
				// idea of the timing of frames received and when frames were
		 ...
		 * the camera, this will mean that this loop will run without ever
		 * waiting on a frame, avoiding any context switching or frame
		 * acquisition time latency.
		 * <p/>
		 * If you find that this is using more CPU than you'd like, you should
		 * probably decrease the FPS setting above to allow for some idle time
		 * in between frames.
		 */
		@Override
		public void run() {
			...
			while (true) {
				synchronized (mLock) {
					...
					outputFrame = new Frame.Builder().setImageData(mPendingFrameData, mPreviewSize.getWidth(), mPreviewSize.getHeight(), ImageFormat.NV21)
							.setId(mPendingFrameId).setTimestampMillis(mPendingTimeMillis).setRotation(mRotation).build();

					// Hold onto the frame data locally, so that we can use this
					// for detection
					// below. We need to clear mPendingFrameData to ensure that
					// this buffer isn't
					// recycled back to the camera before we are done using that
					// data.
