...
import com.vasworks.android.util.Triplet;
 ...
 */
public class DdInstructionListActivity extends AppCompatActivity implements DdInstructionListFragment.Callbacks {

	public static final String LOG_TAG = "DdInstructionListActivity";
	
	private DdInstructionListTask mDdInstructionListTask;
	private View mProgressView;
	private DdInstructionListFragment mInstructionList;
	private ArrayList<Triplet> instructionItems = new ArrayList<>();
...
//			getFragmentManager().beginTransaction().add(R.id.payslip_detail_container, fragment).commit();
		...
		mInstructionList = (DdInstructionListFragment) getFragmentManager().findFragmentById(R.id.instruction_list);

		mProgressView = findViewById(R.id.instruction_list_progress);
		mDdInstructionListTask = new DdInstructionListTask(this);
		mDdInstructionListTask.execute((Void) null);
		Log.i(LOG_TAG, "Payslips Items=" + instructionItems);		
	}
	 ...
	 * the user.
	 */
	public class DdInstructionListTask extends AsyncTask<Void, Void, String> {
		
		private DdInstructionListActivity activity;

		public DdInstructionListTask(DdInstructionListActivity activity) {
			super();
		...
		protected String doInBackground(Void... params) {
			...
			try {
				...
				methodParams.add("imalive_pension_ddebit");
				...
				Object val = models.call("execute_kw", MainActivity.db, MainActivity.userId, MainActivity.password, "hr.employee",
						"list_pensioner_ddebit_instructions", methodParams.toArray());
				...
				try {
					...
					for (Object d : data) {
						...
						Log.i(LOG_TAG, "--------------------------------");
						instructionItems.add(new Triplet(item[0].toString(), item[1].toString(), format.format(Double.valueOf(item[2].toString()))));
					}
				} catch (Exception e) {
		...
		protected void onPostExecute(final String msg) {
			mDdInstructionListTask = null;
			mProgressView.setVisibility(View.GONE);
			
			mInstructionList.setListAdapter(new ArrayAdapter<>(activity, android.R.layout.simple_list_item_1, instructionItems.toArray(new Triplet[instructionItems.size()])));
			((BaseAdapter) mInstructionList.getListAdapter()).notifyDataSetChanged();
		}
