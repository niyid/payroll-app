...
/**
 * Wrapper over the TensorFlow API ({@link Graph}, {@link Session}) providing a
 * smaller API surface for inference.
 *
 * <p>
 * See
 * tensorflow/examples/android/src/org/tensorflow/demo/TensorFlowImageClassifier
 * .java for an example usage.
 */
public class TensorFlowInferenceInterface {
	...
	public void run(String[] outputNames, boolean enableStats) {
		...
		try {
			...
			Log.e(TAG, "Failed to run TensorFlow inference with inputs:[" + TextUtils.join(", ", feedNames) + "], outputs:[" + TextUtils.join(", ", fetchNames)
					+ "]");
			throw e;
		} finally {
	...
	/**
	 * Given a source array with shape {@link dims} and content {@link src},
	 * copy the contents into the input Tensor with name {@link inputName}. The
	 * source array {@link src} must have at least as many elements as that of
	 * the destination Tensor. If {@link src} has more elements than the
	...
	// Methods for taking a native Tensor and filling it with src from Java
	// native IO buffers.

	/**
	 * Given a source buffer with shape {@link dims} and content {@link src},
	 * both stored as <b>direct</b> and <b>native ordered</b> java.nio buffers,
	 * copy the contents into the input Tensor with name {@link inputName}. The
	 * source buffer {@link src} must have at least as many elements as that of
	 * the destination Tensor. If {@link src} has more elements than the
	 * destination has capacity, the copy is truncated.
	 */
	...
	/**
	 * Read from a Tensor named {@link outputName} and copy the contents into a
	 * Java array. {@link dst} must have length greater than or equal to that of
	 * the source Tensor. This operation will not affect dst's content past the
	...
	/**
	 * Read from a Tensor named {@link outputName} and copy the contents into
	 * the <b>direct</b> and <b>native ordered</b> java.nio buffer {@link dst}.
	 * {@link dst} must have capacity greater than or equal to that of the
	 * source Tensor. This operation will not affect dst's content past the
	 * source Tensor's size.
	 */
	...
	private void loadGraph(InputStream is, Graph g) throws IOException {
		...
		// TODO(ashankar): Can we somehow mmap the contents instead of copying
		// them?
